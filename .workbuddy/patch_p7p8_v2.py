# -*- coding: utf-8 -*-
"""Apply phase 7+8: structured _render_msg + smart _render_input_fallback."""
import sys, re
...
def H(s):
    """Convert a string containing literal \\uXXXX escapes into actual characters."""
...
# ============================================================
NEW_MSG = H(r'''def _render_msg(msg):
    ...
    # Inline tool_calls (OpenAI format)
...
# Fix the `else:` indent issue (was 12 spaces instead of 8)
...
assert start_pos > 0 and end_pos > start_pos, f"start={start_pos}, end={end_pos}"
...
m = old_fallback_pattern.search(content)
if m:
    old = m.group(0)
    NEW_FALLBACK = H(r'''def _render_input_fallback(attrs: dict, key: str) -> None:
    """Render an input/output fallback when structured messages are absent.
