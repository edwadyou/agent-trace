# -*- coding: utf-8 -*-
"""One-shot reapplication: translations + flowchart + body wrap."""
import sys
...
# Phase 2: UI strings (note: many are still English at this point)
ui_rules = [
...
for old, new in ui_rules:
    ...
    if c:
        ...
        miss.append(old[:60])
print("Phase 2:", n2, "subs,", len(miss), "misses")
for m in miss[:10]:
    print("  miss:", repr(m))


# Phase 3: Flowchart helpers insertion
helpers = r'''# ---------------------------------------------------------------------------
...
# Phase 4: View-mode radio (place AFTER with hdr_r block ends)
...
# Phase 5: Wrap body.  Note: at this point, "Span Detail" -> "Span 详情" and
# "Metadata" -> "元数据面板" are NOT yet in src (Phase 2 applied those to
# separate `#### 🔍 Span Detail` / `#### 📊 Metadata` literals, not this one).
# Actually Phase 2 had rules for "#### 🔍 Span Detail" and "#### 📊 Metadata"
# which translated them. Let me verify and use the appropriate literal.

# Find the body section
body_idx = src.find("col_tree, col_detail, col_meta = st.columns")
assert body_idx > 0, "Phase 5 body marker missing"
end_idx = src.find("_render_metadata_panel(kpi, sel_spans", body_idx)
assert end_idx > 0, "Phase 5 metadata panel call missing"
end_idx = src.find("\n", end_idx) + 1

old_body = src[body_idx:end_idx]
...
src = src[:body_idx] + new_body + src[end_idx:]
