...
# Find the exact block: from "# --- Input block (collapsible)" comment header
# up through (but not including) the "# --- Token breakdown" section header.
start = src.find("# --- Input block (collapsible)")
end = src.find("# --- Token breakdown")
assert start > 0 and end > start

# The current block has 4 spaces of indentation; we'll keep the same.
old_block = src[start:end]

# Note: this block has 4 leading spaces because it's inside def _render_run_tab(...).
# In the new side-by-side version we also keep 4-space indent.
