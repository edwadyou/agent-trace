# -*- coding: utf-8 -*-
"""Re-translate canonical.py and langchain.py."""
import sys
sys.stdout.reconfigure(encoding="utf-8")

# canonical.py: SPAN_KINDS labels
cp = r"D:\my-projects\agent-monitor\viewer\canonical.py"
content = open(cp, encoding="utf-8").read()
repl = [
    ("\"LLM call\"", "\"\u5927\u6a21\u578b\u8c03\u7528\""),
    ("\"Chain step\"", "\"\u94fe\u8def\u6b65\u9aa4\""),
    ("\"Tool call\"", "\"\u5de5\u5177\u8c03\u7528\""),
...
for old, new in repl:
    if old in content:
        ...
        print(f"  OK canonical: {old}")
open(cp, "w", encoding="utf-8", newline="\n").write(content)
print("canonical.py done")

# langchain.py: many rules
lp = r"D:\my-projects\agent-monitor\viewer\naming\langchain.py"
# First rewrite the whole file
LANGCHAIN = """\"\"\"LangChain / LangGraph span-name translation rules.
...
def _parallel_label(_raw, m, _kind):
    \"\"\"``RunnableParallel<web_search,knowledge_base,social_listening>`` -> ``\u591a\u4efb\u52a1\u5e76\u884c: web_search, knowledge_base, social_listening``.\"\"\"
...
LANGCHAIN_RULES = [
    ...
    NamingRule(re.compile(r\"^RunnableWithMessageHistory$\"), \"\u5e26\u5386\u53f2\u6d88\u606f\"),
...
\"\"\"
open(lp, "w", encoding="utf-8").write(LANGCHAIN)
