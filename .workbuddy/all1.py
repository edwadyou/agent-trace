# -*- coding: utf-8 -*-
"""Comprehensive viewer.py patcher: all changes in one pass."""
import sys, re
...
# ===== 1. Chinese translations (Phase 1+2) =====
T1_RULES = [
    ("Tool Agent", "\u5de5\u5177\u578b\u667a\u80fd\u4f53"),
    ("Retrieval Chain", "\u68c0\u7d22\u94fe"),
    ("LLM Chain", "LLM \u8c03\u7528\u94fe"),
...
print("Phase 1: trace_label translations")
content = replace_once(content, T1_RULES[0][0], T1_RULES[0][1], "Tool Agent")  # first to apply
for old, new in T1_RULES[1:]:
    content = replace_once(content, old, new, old[:30])
...
print("Phase 2: UI string translations")
for old, new in T2_RULES:
    content = replace_once(content, old, new, old[:40])

open(p, "w", encoding="utf-8", newline="\n").write(content)
print(f"After Phase 1+2: {len(content.encode('utf-8'))} bytes")