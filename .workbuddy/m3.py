...
# ===== Phase 4: View-mode switcher (we'll delete this later in Phase 11) =====
...
assert body_start > 0 and body_end_search > body_start
...
print(f"Phase 5: body wrap inserted")
...
old_msg_start = "def _render_msg(role: str, content: object) -> None:"
# Find end of _render_msg (next def or class at 0 indent)
msg_start_idx = content.find(old_msg_start)
# Walk to find the end - look for next blank line followed by def at 0 indent
i = msg_start_idx
while i < len(content):
    if content[i:i+4] == "\n\n\ndef " or content[i:i+4] == "\n\n\n# ":
        break
    i += 1
old_msg = content[msg_start_idx:i]
...
'''
content = content[:msg_start_idx] + new_msg + content[i:]
...
content = replace_once(content, old_call_in, new_call_in, "msg call (input)")
...
content = replace_once(content, old_call_out, new_call_out, "msg call (output)")
...
content = replace_once(content, old_body, new_body, "msg-body CSS")
